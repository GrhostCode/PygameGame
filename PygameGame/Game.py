import pygame, random, json, Move
import math
import os

def run_simulation(playersAmmount=10, foodAmmount=20, duration_ms=None):
    pygame.init()
    ...
    class player:
        def __init__(self, X, Y, W, H):
            ...
            self.tx = X
            self.ty = Y
            self.speed = 5 * runSpeed
            self.food = 50
            self.mating = False
            self.foodDistance = 150
    ...
    def make_offspring(a, b):
        ...
        child.speed        = max(1, avg_speed * random.uniform(0.06, 1.5))
        child.foodDistance = int(max(10, avg_dist * random.uniform(0.6, 2.0)))
        child.food         = 50
    ...
    if not load:
        ...
        data  = [json.loads(line) for line in read_file(os.path.join(base, "plrSave.json")).splitlines()]
        plrList = [player(d["x"], d["y"], d["w"], d["h"]) for d in data]
        fdata = [json.loads(line) for line in read_file(os.path.join(base, "foodSave.json")).splitlines()]
        foodList = [food(d["x"], d["y"], d["w"], d["h"]) for d in fdata]
    ...
    while run:
        ...
        if plrList:
            ...
            hud = f"Players: {len(plrList)}, Average age: {avg_age:.1f}, MaxAge: {MAX_AGE}"
        else:
            hud = f"Players: 0, Average age: 0.0, MaxAge: {MAX_AGE}"
    ...
    write_file(os.path.join(base, "plrSave.json"), "\n".join(json.dumps(p) for p in data))
    ...
    write_file(os.path.join(base, "foodSave.json"), "\n".join(json.dumps(f) for f in food_data))
